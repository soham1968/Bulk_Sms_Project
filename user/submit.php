...
if (isset($_REQUEST['submit_quick'])) {
    ...
    $cred = "SELECT * FROM `dashboard` WHERE `username` = '$username'";
    $che = mysqli_query($con, $cred);
    $ar = mysqli_fetch_array($che);
    $credit_left = $ar['credit_left'];
    // echo $credit_left;

    if ($credit_left >= $msg_credit) {
        $sql = "INSERT INTO 
        ...
        if ($res) {
            $sqlm = "UPDATE `dashboard` SET `total_msg`=`total_msg`+ $msg_credit, `credit_used`=`credit_used` +  
            $msg_credit,`credit_left`=`credit_left`- $msg_credit  WHERE  `username` = '$username'";
            $resa = mysqli_query($con, $sqlm);

            $_SESSION['showQuick'] = "Your Message has been stored";
            header("Location: quick.php");
        } else {
            $_SESSION['showQuickError'] = "Your Message has not been accepted please try again";
            header("Location: quick.php");
        }
    } else {
        $_SESSION['showQuickError'] = "Your Credit is not enough to send message";
        header("Location: quick.php");
    }
}
...
if (isset($_REQUEST['submit_csv'])) {
    ...
    if ($credit_left >= $msg_credit) {
        ...
        $_SESSION['showCsvError'] = "Your Credit is not enough to send message";
        header("Location: csv.php");
    }
}
...
if (isset($_REQUEST['group_msg'])) {
    ...
    if ($credit_left >= $msg_credit) {
        $sql = "INSERT INTO  `sms_details`( `username`, `msg_type`, `group_name`, `template`, `language`, `msg_id`, `msg_body`, `mobile_numbers`, `sender_name`, `service`,                 
        `delivered_msg`,`rejected_msg`,  `credit_used`, `scheduled`, `status`)
        ...
        $_SESSION['showGroupError'] = "Your Credit is not enough to send message";
        header("Location: grmes.php");
    }
}
